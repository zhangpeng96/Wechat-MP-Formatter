var app = new Vue({
  ...
  data: function () {
    return {
      ...
      editor: null,
      builtinFonts: [
        { label: '衬线', value: "Optima-Regular, Optima, Cambria, Cochin, Georgia, Times, 'Times New Roman', serif, 'PingFang SC'"},
      ...
      themeOption: [
        ...
        { label: 'phytalk', value: 'phytalk', author: '喵'},
        { label: 'lupeng', value: 'lupeng', author: '鲁鹏'}
      ],
      styleThemes: {
        default: defaultTheme,
        phytalk: phytalkTheme,
        lupeng: lupengTheme
      },
  ...
  methods: {
    renderWeChat: function (source) {
      // console.log(source)
      var output = marked(source.replaceAll('\\','\\\\'), { renderer: this.wxRenderer.getRenderer() })
    ...
    refresh: function () {
      if(window.MathJax){
        console.log('???')
        // window.MathJax.Hub.Queue(["Typeset", window.MathJax.Hub,this.output]);
        setTimeout( function () {
          window.MathJax.Hub.Queue(["Typeset", window.MathJax.Hub])}, 10)
      }
      this.output = this.renderWeChat(this.editor.getValue())
      // window.MathJax.Hub.Rerender()
      // window.MathJax.Rerender()
      // console.info(window.MathJax)
      // console.info(this.MathJax)
